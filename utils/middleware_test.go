...
func TestLogger(t *testing.T) {
	...
	if assert.Equal(t, 2, len(hook.Entries), "There should be valid ulid in request") {
		assert.Equal(t, logrus.InfoLevel, hook.Entries[0].Level, "Wrong first log entry level")
		...
		assert.Equal(t, logrus.InfoLevel, hook.Entries[1].Level, "Wrong second log entry level")
...
func TestCheckJWT(t *testing.T) {
	...
	for _, ex := range examplesJWT {
		t.Run(ex.name, func(t *testing.T) {
			...
			if len(ex.result.logMessage) > 0 && assert.Equal(t, 1, len(hook.Entries), "Should have log entry") {
				assert.Regexp(
					t,
			...
			assert.Equal(t, ex.result.statusCode, w.Result().StatusCode, "Incorrect response code")
			b, err := ioutil.ReadAll(w.Result().Body)
			if err != nil {
				t.Fatal(err)
			}
			assert.Regexp(t, ex.result.body, string(b), "Incorrect response body")
